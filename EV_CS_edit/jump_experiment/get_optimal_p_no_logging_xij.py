...
'''
@Project ：EV_CS_edit 
@File    ：get_optimal_p_no_logging.py
@Author  ：xxuanZhu
@Date    ：2021/5/27 16:05
@Purpose : get all cs optimal p with no logging.
'''
...
def get_max_revenue(dist, region_num, cs_num, priceIndex, price, vehicle_num, flag, results, price_section_index,
    ...
    print("---------------下面开始求价格区间为：[", results[price_section_index - 1], ",", results[price_section_index],
              "]的最大收益---------------")
    model.p = Var(bounds=(results[price_section_index - 1], results[price_section_index]))
    ...
    # model.ni = ConstraintList()
    # for i in model.region:
    #     model.ni.add(sum(model.f[i, j] * vehicle_num[i] for j in model.CS) - vehicle_num[i] == 0.0)
...
def get_next_jump_p(dist, vehicle_num, price, expression_dict, fij_symbol_list, q_symbol_list, lamuda_symbol_list,
    ...
    if last_lower_0_flag:
        ...
        for j in range(config.cs_num):
            ...
            for i in range(config.region_num):
                expr += (fij_symbol_list[i][j] * vehicle_num[i])
            ...
            if p_equal_flag:
                ...
                if str(head_fij)[0] == 'f':  # 如果上一轮确定的p是因为fxx变化，说明上一轮求p时fij会从大于0变成小于0，当求下一个跳跃点时需要去掉
                    ...
                    if int(int_j) == change_cs:
                        expression_dict[expr_f] = vehicle_num[int_i] * (
                                    p + q_symbol_list[int_j] + dist[int_j][int_i] + fij_symbol_list[int_i][int_j] * vehicle_num[int_i]) - \
                                                  lamuda_symbol_list[int_i]
                    else:
                        expression_dict[expr_f] = vehicle_num[int_i] * (
                                        price[int_j] + q_symbol_list[int_j] + dist[int_j][int_i] + fij_symbol_list[int_i][int_j] * vehicle_num[int_i]) - \
                                                      lamuda_symbol_list[int_i]
            ...
            while copy_last_min_p_fij_queue:
                ...
                if p_equal_flag:
                    ...
                    if str(head_fij)[0] == 'f':  # 如果上一轮确定的p是因为fxx变化，说明上一轮求p时fij会从大于0变成小于0，当求下一个跳跃点时需要去掉
                        ...
                        if int(int_j) == change_cs:
                            expression_dict[expr_f] = vehicle_num[int_i] * (
                                    p + q_symbol_list[int_j] + dist[int_j][int_i] + fij_symbol_list[int_i][int_j] *
                                    vehicle_num[int_i]) - \
                                                      lamuda_symbol_list[int_i]
                        else:
                            expression_dict[expr_f] = vehicle_num[int_i] * (
                                    price[int_j] + q_symbol_list[int_j] + dist[int_j][int_i] + fij_symbol_list[int_i][
                                int_j] * vehicle_num[int_i]) - \
                                                      lamuda_symbol_list[int_i]
                        state_dict[fij_symbol_list[int_i][int_j]] = 1
                        # 加入后检查是否存在矛盾
                        # logging.debug("添加后的方程组表示为：%s", expression_dict)
    ...
    if lower_0_flag == True:
        ...
        for j in range(config.cs_num):
            expr = 0
            for i in range(config.region_num):
                expr += (ans[fij_symbol_list[i][j]] * vehicle_num[i])
            q_symbol_list[j] = expr
        fluctuate_i_j = []
        if p_equal_flag:
            copy_last_min_p_fij_queue = last_min_p_fij_queue.copy()
                ...
                if (ans[fij_symbol_list[i][j]] == 0):
                    if fluctuate_i_j != []:
                        ...
                        for k in range(config.region_num):
                            ...
                            if j == change_cs:
                                expr = vehicle_num[i] * (dist[j][i] + q_symbol_list[j] + p) - ans[lamuda_symbol_list[k]]
                            else:
                                expr = vehicle_num[i] * (dist[j][i] + q_symbol_list[j] + price[j]) - ans[lamuda_symbol_list[k]]
    ...
    # 拷贝这轮的跳跃点价格及对应的fij变化
    # logging.info("最后的跳跃点为：%s", min_p)
    # logging.info("最后的跳跃点所对应的fij为：%s", min_P_fij)
...
def get_optima_p(price, change_cs, region, dist, vehicle_num, minimize_res_list, strategy_vector, lun, start_flag,
    ...
    for i in range(config.region_num):
        ...
        for j in range(config.cs_num):
            expression_dict[expr_n] += (fij_symbol_list[i][j] * vehicle_num[i])
    ...
    for i in range(config.region_num):
        for j in range(config.cs_num):
            expr_f = str(i) + str(j)
            if j == change_cs:
                expression_dict[expr_f] = vehicle_num[i] * (p + q_symbol_list[j] + dist[j][i] + fij_symbol_list[i][j] * vehicle_num[i]) - lamuda_symbol_list[i]
            else:
                expression_dict[expr_f] = vehicle_num[i] * (price[j] + q_symbol_list[j] + dist[j][i] + fij_symbol_list[i][j] * vehicle_num[i]) - lamuda_symbol_list[i]
            # 将状态全部初始化为1，表示fij>0
    ...
    if revenue_max_p_list != []:
        ...
        print("当前充电站为", change_cs, "时，最优p下的fij配置为：", flow_list_list[max_index])
