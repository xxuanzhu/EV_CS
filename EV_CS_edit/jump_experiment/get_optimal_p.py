...
def get_max_revenue(dist, region_num, cs_num, priceIndex, price, vehicle_num, flag, results, price_section_index,
    ...
    model.CS = range(M)
    logging.info("---------------下面开始求价格区间为：[%s, %s]的最大收益---------------", results[price_section_index - 1],  results[price_section_index],)
    ...
    model.obj = Objective(expr=sum(model.p[k] * sum(vehicle_vector[i] * model.f[i, change_cs] for i in model.region) for k in model.priceIndex), sense=maximize)
    ...
    for i in model.region:
        for j in model.CS:
            ...
            if j == change_cs:
                model.lagrange.add(
                    model.p[0] + sum(model.f[k, j] * vehicle_vector[k] for k in model.region) + dist_vector[j][
                        i] +
...
def get_next_jump_p(dist, vehicle_num, price, expression_dict, fij_symbol_list, q_symbol_list, lamuda_symbol_list,
                    ...
                    last_min_p_fij_queue, state_dict, flag, p_equal_flag, p, strategy, priceIndex, change_cs, start_flag, flow_list_list, p_max):
    # print("是否出现相等的跳跃点：\n", p_equal_flag)
        ...
        if item[1][0] != []: # 首先不能为空
            if '/' in item[1][0]:  # 如果待选值是分数
                ...
                if round(int(nums_arr[0]) / int(nums_arr[1])) > price[change_cs]:  # 首先待选值（分数）要大于当前的price
                    logging.info("待选值 %s 不为空，待选值为分数， 大于当前的price %s", round(int(nums_arr[0]) / int(nums_arr[1])), price[change_cs])
                    # 拿到当前的最小值
                    head = min_p.popleft()
...
def get_optima_p(price, change_cs, region, dist, vehicle_num, minimize_res_list, strategy_vector, lun, start_flag, p_max):

    region_num = config.region_num
    cs_num = config.cs_num
    priceIndex = 1  # 公司控制priceIndex个充电站
    revenue_list = []  # 多个跳跃点区间中最优价格list
    price_section_index = 1  # 价格区间索引
    result_jump_p = []  # 求得的跳跃点list
...
if __name__ == "__main__":
    ...
    while np.linalg.norm(last_cs_optimal_p_list - price) > 1e-8:
        ...
        print("第", lun, "轮更新开始：")
        for cs in range(config.cs_num):
            p, r, optimal_flow_list, flag, start_flag = get_optima_p(price, cs, region, dist, vehicle_num, minimize_res_list, strategy_vector, lun, start_flag, p_max)
            if flag:
                price[cs] = p
                revenue_list[cs] = r
