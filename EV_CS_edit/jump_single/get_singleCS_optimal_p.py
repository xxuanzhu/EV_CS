...
'''
@Project ：EV_CS_edit 
@File    ：get_singleCS_optimal_p.py
@Author  ：xxuanZhu
@Date    ：2021/6/4 10:01 
@Purpose : 固定其他充电站，确定其中一个充电站的最优定价
'''
...
def get_next_jump_p( dist, vehicle_num, copy_price, expression_dict, fij_symbol_list, q_symbol_list, lamuda_symbol_list, jump_point_list,
                     ...
                     last_min_p_fij_queue, state_dict, flag, p_equal_flag, p, strategy,  change_cs, p_max, flag_each_jump_step):
    """
    :param dist: 区域到充电站的距离
    :param vehicle_num: 每个区域的车辆数
    :param copy_price: 充电站当前的价格
    :param expression_dict: 方程组表示
    :param fij_symbol_list: fij的符号表示
            ...
            if p_equal_flag:  # 如果出现了跳跃点波动的情况
                ...
                if str(head_fij)[0] == 'f':  # 如果因为fij变化，说明此轮fij会从大于0变成等于0，需要去掉该方程
                    ...
                    if int(int_j) == change_cs:
                        ...
                        expression_dict[expr_f] = fij_symbol_list[int_i][int_j] + q_symbol_list[int_j] + dist[int_j][
                            int_i] + copy_price[int_j] - lamuda_symbol_list[int_i]
                    ...
                    if str(head_fij)[0] == 'f':  # # 如果因为fij变化，说明此轮fij会从大于0变成等于0，需要去掉该方程
                        ...
                        if int(int_j) == change_cs:
                            ...
                            expression_dict[expr_f] = fij_symbol_list[int_i][int_j] + q_symbol_list[int_j] + \
                                                      dist[int_j][
                                                          int_i] + \
                                                      copy_price[int_j] - lamuda_symbol_list[int_i]
                        state_dict[fij_symbol_list[int_i][int_j]] = 1

                        # 加入后检查是否存在矛盾
                        if check_if_conflict(expression_dict,  lamuda_symbol_list, state_dict, copy_price, p, change_cs,head_fij):
    ...
    if lower_0_flag == True:  # 如果存在fij=0，需要计算什么时候fij从=0开始有流量
        ...
        for i in range(config.region_num):
            for j in range(config.cs_num):
                if ans[fij_symbol_list[i][j]] == 0:  # fij流量为0,且正好是此fij导致跳跃点振荡的情况，跳过此轮求解
                    if fluctuate_i_j != [] and i == fluctuate_i_j[0] and j == fluctuate_i_j[1]:
                        ...
                        for k in range(config.region_num):  # 求解 qj+dij+p = lamudi_i
                            ...
                            if j == change_cs:
                                ...
                                expr = dist[j][i] + q_symbol_list[j] + copy_price[j] - ans[lamuda_symbol_list[k]]
                            ineq_expression_dict[fij_name] = expr

        # 把fij何时等于0的点解出来
        for item in ineq_expression_dict.items():
            if 'p' in str(item[1]):  # 这个地方说明是有些fij规定为某个值，不随p变化
...
if __name__ == "__main__":
    ...
    for i in range(region_num):
        for j in range(cs_num):
            ...
            if j == change_cs:
                ...
                expression_dict[expr_f] = fij_symbol_list[i][j] + q_symbol_list[j] + dist[j][i] + copy_price[j] - \
                                          lamuda_symbol_list[i]
            # 将状态全部初始化为1，表示fij>0
    ...
    while end_flag:
        result_p, last_lower_0_flag,  last_min_p_queue, last_min_p_fij_queue, flag, expression_dict, p_equal_flag, end_flag = get_next_jump_p(
            ...
            state_dict, flag, p_equal_flag, p, strategy,  change_cs,  p_max, flag_each_jump_step)
        price_section_index += 1
