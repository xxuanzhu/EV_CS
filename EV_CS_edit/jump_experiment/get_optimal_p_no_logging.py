...
def get_max_revenue(dist, region_num, cs_num, priceIndex, price, vehicle_num, flag, results, price_section_index,
    ...
    # model.f = Var(model.region, model.CS, bounds=(0, None))  # 车流量
...
def get_next_jump_p(dist, vehicle_num, price, expression_dict, fij_symbol_list, q_symbol_list, lamuda_symbol_list,
    ...
    if revenue == 0:
        return result_jump_p[
               price_section_index], last_lower_0_flag, last_great0_to_lower_0_flag, last_min_p_queue, last_min_p_fij_queue, flag, expression_dict, p_equal_flag, False, start_flag
    ...
    return result_jump_p[price_section_index], last_lower_0_flag, last_great0_to_lower_0_flag, last_min_p_queue, last_min_p_fij_queue, flag, expression_dict, p_equal_flag, True, start_flag
...
def get_optima_p(price, change_cs, region, dist, vehicle_num, minimize_res_list, strategy_vector, lun, start_flag,
    ...
    if revenue_max_p_list != []:
        ...
        ff = copy.deepcopy(flow_list_list[max_index])
        for i in range(config.region_num):
            print("当前充电站为", change_cs, "时，最优p下的fij配置为：", sum(ff[i]))
            if sum(ff[i]) > 1.0:
                print("超过啦！！！！")
...
if __name__ == "__main__":
    ...
    optimal_flow_list = []
    price = [10 for i in range(config.cs_num)]
