...
from new_experiment_company_max.EV_Equilibrium_cs_change import EvEquilibrium
...
if __name__ == "__main__":
    ...
    price = [20, 20, 20, 20, 20, 20, 20, 20, 20, 20,
             20, 20, 20, 20, 20, 20, 20, 20, 20, 20 ]  # 前四个为一个公司控制的充电桩，后6个价格固定

    all_revenue_list = [[] for i in range(config.cs_num)]  # 所有充电站的revenue列表

    own_cs_revenue_list = []  # 公司控制的充电站收益之和列表
    ...
    for cs in range(config.cs_num):
        ...
        own_cs_revenue_list.append(own_cs_revenue)
    ...
    print("当前充电站数量为：", config.cs_num, "的情况下,", "其收益之和list为：", own_cs_revenue_list)
    for i in range(4):
        print("我公司控制的cs ", i, " 的定价为：", price[i])
    file = open("change_cs.txt", "a+")
    file.write(str(own_cs_revenue_list))
    file.close()
    ...
    # plt.xlabel("num")
    # plt.ylabel("revenue")
    # plt.plot(range(len(own_cs_revenue_list)), own_cs_revenue_list, color=color[0],
    #          label='cs is ' + str(symbol + 5) + " revenue curve")
